...
func TestNeonExecutionBootstrap(t *testing.T) {
	...
	tenantID := "test-tenant-exec-" + suffix
	defer cleanupExecutionRows(t, pool, tenantID, userID)

	identityService := identity.NewService(identityStore)
	if err := identityService.EnsurePersonalTenant(ctx, auth.Principal{
		UserID:          userID,
	...
	fetched, err := executionStore.GetRun(ctx, tenantID, userID, run.ID)
	if err != nil {
	...
	// Scoped reads.
	if _, err := executionStore.GetRun(ctx, "other-tenant", userID, run.ID); err != execution.ErrRunNotFound {
		t.Fatalf("expected ErrRunNotFound for cross-tenant get, got %v", err)
	}
	runs, err := executionStore.ListRuns(ctx, tenantID, userID)
	if err != nil {
...
func assertExecutionOrphanRejected(t *testing.T, ctx context.Context, pool *pgxpool.Pool) {
	...
	_, err := pool.Exec(ctx, `
		INSERT INTO execution_runs (
			id, tenant_id, user_id, problem_id, language, entrypoint,
			files, status, created_at
		)
		VALUES ($1, 'missing-tenant', 'missing-user', '', 'python', 'x.py', '[]'::jsonb, 'queued', now())
...
func cleanupExecutionRows(t *testing.T, pool *pgxpool.Pool, tenantID, userID string) {
	t.Helper()
	...
	statements := []struct {
		...
		{sql: `DELETE FROM execution_runs WHERE tenant_id = $1 AND user_id = $2`, args: []any{tenantID, userID}},
