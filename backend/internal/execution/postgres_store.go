...
func (s *PostgresStore) EnsureSchema(ctx context.Context) error {
	statements := []string{
		...
		`CREATE INDEX IF NOT EXISTS idx_execution_runs_scope_created_at ON execution_runs (tenant_id, user_id, created_at DESC)`,
		`DO $$
		BEGIN
			IF NOT EXISTS (
				SELECT 1
...
func (s *PostgresStore) CreateRun(ctx context.Context, run Run) error {
	...
	`, run.ID, run.TenantID, run.UserID, run.ProblemID, run.Language, run.Entrypoint,
...
func (s *PostgresStore) UpdateRun(ctx context.Context, run Run) error {
	...
	`, run.ID, run.TenantID, run.UserID, string(run.Status), run.ExitCode,
...
func (s *PostgresStore) GetRun(ctx context.Context, tenantID, userID, id string) (Run, error) {
...
func (s *PostgresStore) ListRuns(ctx context.Context, tenantID, userID string) ([]Run, error) {
