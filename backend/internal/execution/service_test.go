...
func TestSubmitRunPasses(t *testing.T) {
	...
	if run.TenantID != "tenant-1" || run.UserID != "user-1" {
		t.Fatalf("unexpected scope: %s/%s", run.TenantID, run.UserID)
	}
...
func TestGetRunScopedToTenant(t *testing.T) {
	...
	otherCtx = tenant.WithScope(otherCtx, tenant.Scope{TenantID: "tenant-2"})
