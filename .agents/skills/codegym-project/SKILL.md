...
- Generation: `generation.Orchestrator` composes the scoped memory profile with the provider-neutral `Generator` seam. Google direct GenAI configs use `generation/gemini` and Gemini's native Interactions API with JSON Schema structured output; Vercel AI Gateway and other OpenAI-shaped configs use `generation/openaicompat`. `POST /api/v1/generate` serves MCQ sets with structural validation and one bounded repair retry, including adapter-level malformed JSON before schema validation; it answers 503 when `CODEGYM_GENAI_API_KEY` is unset. MCQ prompt text mirrors `docs/ai-prompts/05-mcq-marathon.md` — keep them in sync.
...
- CORS: backend CORS uses explicit origins and one-label wildcard origins. Render should allow the production frontend plus Kevin's Vercel preview pattern: `https://code-gym-rho.vercel.app,https://*-kvn8888s-projects.vercel.app`.
- Secrets: Doppler is the preferred local secret runner; `NEON_CONNECTION_STRING` is checked before `DATABASE_URL`. GenAI: `CODEGYM_GENAI_BASE_URL` / `CODEGYM_GENAI_API_KEY` (alias `AI_GATEWAY_API_KEY`) / `CODEGYM_GENAI_MODEL`. The default direct Gemini pairing is `https://generativelanguage.googleapis.com/v1beta` plus `gemini-3.5-flash`; Vercel AI Gateway requires `https://ai-gateway.vercel.sh/v1` plus a vendor-prefixed model slug.
