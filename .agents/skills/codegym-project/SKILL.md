...
- Append-only memory events are deterministic evidence. `generation.ProfileSynthesizer`
  ...
  MCQ skips are neutral `question_skipped` evidence. The UI may reveal the
  correct answer after Skip, but that reveal never becomes learner correctness;
  legacy `answer_incorrect` events with `skipped: true` are canonicalized before
  synthesis. Profile notes are a model-managed desired state: synthesis reuses
