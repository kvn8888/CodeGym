...
description: Architecture for the agent-driven learning flow. Covers the Codex agent question modal, Chat interface, MCQ Marathon format, recommendation engine, Gemini-powered user profile memory, and backend generation pipeline. Use when implementing or modifying any learning feature, user profile system, the multi-step question modal, chat, or marathon.
---
...
1. **Problem Generation** — Codex interviews the user, then generates a
   LeetCode-style coding problem tailored to their skill level.
2. **Chat** — A conversational interface with Codex for learning goals,
...
│                                                                 │
│  Agents:                                                        │
│  ├─ Codex: question generation, problem creation, chat, recs  │
...
2. Backend calls Codex to produce a **recommendation JSON** and questions.
3. Frontend shows the QuestionModal (max 3 questions per series).
4. After answers, Codex generates the problem (YAML + skeleton + tests).
5. Codex's recommendation may suggest Marathon instead (see §4).

### Recommendation JSON

After analyzing the user's prompt + memory, Codex returns:
...
The Chat page provides a conversational interface with Codex whose primary
mission is to:
...
- `POST /api/v1/chat` — Send a message, receive Codex's response + updated
...
After the QuestionModal in the Generate flow, Codex produces a
...
- `POST /api/v1/generate/questions` — Codex generates clarifying questions.
- `POST /api/v1/generate` — Codex generates problem from prompt + answers.
...
2. **Chat page + sidebar nav** — conversational Codex interface
3. **Marathon page + HelpFlashcard** — timed MCQ with concept help
4. **POST /api/v1/generate/questions** — backend, Codex generates questions
5. **POST /api/v1/generate** — backend, Codex generates problem
...
9. **Recommendation engine** — Codex returns recommendation JSON after questions
