...
- Branch: `codegym-v2`.
- Frontend: React 19 + Vite 7, Storybook 10, Motion/Framer-style animations, Monaco editor, and mock-friendly app routes. Vitest 3 + Testing Library provide the jsdom unit/component harness; tests use the same Theme and Tooltip providers as the app and force `VITE_USE_MOCK_API=false` so fetch behavior must be stubbed explicitly. The visual system is a quiet operational workspace: 24px routine page titles, flat border-led surfaces, compact rows, a 216px desktop sidebar, and a dedicated mobile navigation sheet. Sidebar nav is Dashboard, New practice, History, Memory, Settings (Marathon is not a nav item). Dashboard composes active/recent sessions with memory-derived recommendations; Memory composes the profile, event history, focus queue, skills, and notes; `/generate` is labeled New Practice and supports MCQ, DSA coding, and dedicated Interview formats. Interview modes are coding, system design, behavioral, and open coaching; all pass through the same intake gate, create a resumable `interview` practice session, and open `/interviews/:sessionId`. Dashboard and History restore active or completed interview transcripts. MCQ starts a marathon session; coding runs the same intake gate, requests a memory-aware `kind: problem`, persists the returned personal problem, creates a workspace session, and opens its editor. Generated Python problems accept only typed cases and a reference function from the model; the server alone builds the fixed `CODEGYM_RESULT` hidden-test harness. Generated records are visible only to their workspace owner, while seeded Two Sum remains globally readable as the execution smoke fixture. Coding submissions emit governed compact events without code or tests, preserve the terminal result when memory synthesis fails, and expose a retryable memory-update state; new problem generation is blocked while an explicit failed/pending memory update would make personalization stale. New Practice creates a complete durable MCQ snapshot and hands it to Marathon, which restores the exact question, timer, and draft selection. Bare `/marathon` redirects to `/generate`; only launch state or `?session=` may enter the run UI. History (`/`) surfaces active coding drafts, persisted interviews, and MCQ runs. The coding workspace restores the selected active `workspace` session, autosaves files and basic editor state after 800ms, and runs real `/submissions` polling with per-test results. OAuth-aware route loaders wait for authenticated state so browser back-forward-cache restoration cannot preserve a stale missing-token response. Marathon Skip emits the neutral `question_skipped` event (with `skipped: true`) so memory/personalization does not treat revealed answers as misses, while Exit saves the active session and flushes a `session_exited` event before leaving. MCQ generation, memory reflection, and the combined Next Round workflow use a compact top-right deterministic activity surface backed by authenticated resumable SSE. Finished and Next Round await full profile synthesis, and a backend prerequisite prevents next-round generation until those memory steps have succeeded. Floating chat is limited to active Marathon and problem-detail contexts and uses the same authenticated conversation service as Interview. Settings exposes `/api/v1/me` and lets users save a custom display name.
- Backend: Go service with `auth -> identity -> personal workspace scope -> handler` request path, Auth0 RS256/JWKS bearer-token validation behind `auth.Authenticator`, dev-token auth fallback, Auth0 `sub` to personal workspace bootstrap, optional Auth0 `email`/`name` user-metadata reconciliation, `/api/v1/me` account profile APIs, memory profile/event APIs, session APIs, Neon/Postgres store support, structured LLM profile synthesis, manual profile refresh, and scheduled profile refresh. `workflow_operations` plus append-only `workflow_progress_events` persist backend-owned MCQ generation and memory-reflection steps under the same workspace/user membership. Transactional sequences and `Last-Event-ID` replay make the authenticated `progress` SSE stream reconnectable across server instances; a final allowlisted event carries `terminal=true`, cancellation records a terminal safe reason, and the client falls back to overall request status when streaming is unavailable. One scoped `chat` service backs Interview and contextual coach. `chat_threads` binds a conversation to workspace, user, and practice session; append-only `session_messages` preserves completed/interrupted turns and client-message idempotency, and both cascade with session/account deletion. The provider-neutral `Streamer` emits safe text deltas through `meta|delta|complete|error` SSE; fallback is allowed only before the first emitted delta. Authoritative context is resolved server-side (coding coach includes persisted session drafts or skeleton fallback as `workspace_files`; clients never send source files) and transcripts never enter memory events. Interview completion runs a separate validated assessment, records only coarse learning labels and server-owned counts/duration, preserves terminal completion if memory maintenance fails, and exposes a retry state. The global problem catalog is intentionally not workspace-owned; `CODEGYM_SEED_DEMO=true` idempotently upserts the single `two-sum` public spec, skeleton, server-only hidden tests, and manual reference solution. `/api/v1/problems` exposes only the public representation. `/api/v1/submissions` assembles hidden tests server-side over the workspace-scoped execution store, parses the last `CODEGYM_RESULT` line into per-test results, saves only user files into the associated workspace session, and completes that session on pass. Execution run files are never serialized because persisted bundles can contain hidden tests. A first memory synthesis failure remains an unpersisted empty state; deterministic evidence is not stored as a user-facing profile. Auth0/Google `name` seeds `app_users.display_name` with `display_name_source=oauth`; Settings updates set `display_name_source=user`, and future OAuth metadata must not overwrite user-edited names. Production Auth0 uses the custom API Identifier `https://codegym.onrender.com` (not the Auth0 Management API audience) in matching backend/frontend Doppler configs. The CodeGym SPA requires a `subject_type: user` client grant for that API.
- Execution: `execution.DaytonaRunner` is the only backend file that imports the Daytona SDK. It creates an ephemeral `NetworkBlockAll` sandbox from the language snapshot, uploads home-relative files under `work/`, runs with the language timeout, treats nonzero exit as a normal result, and deletes with an independent cleanup context. Each run logs stage timings (`create_ms`, `upload_ms`, `exec_ms`, `total_ms`) plus exit code. Use `doppler run -p codegym -c dev -- go test ./internal/execution -run TestDaytonaRunner -count=1 -v` for the real passing/failing/blocked-egress gate.
- Generation: `generation.Orchestrator` composes the scoped memory profile with the provider-neutral `Generator` seam. `generation/openaicompat` is the first adapter (OpenAI chat-completions wire format; default base URL is the Vercel AI Gateway). New Practice prepares a workspace/user-scoped `practice_intake` for an unfamiliar normalized topic before MCQ or coding generation. Exact aliases, demonstrated events, profile skills/confidence, and prior completed/skipped intake determine suppression; loose substring matching is forbidden. Pending answers persist immediately and restore on reload. Completed self-report is injected as `self_reported_baseline`, separate from demonstrated memory, which takes precedence. Provider/validation failure exposes Retry and Skip and never fabricates questions; Update baseline sends `restart: true`. New Practice start uses stable `idle|pending|failed` status and blocks double-submit while pending. `POST /api/v1/generate` serves structurally validated single-select, multi-select, free-response, or mixed MCQ sets with one bounded repair retry. Question format is an LLM decision made independently for each concept; it is not a New Practice control or request field. Multi-select full correctness still uses exact-set matching, but Marathon assigns every answered MCQ a normalized `score` in `[0,1]`. Multi-select partial credit uses `correctSelected / totalCorrect - 0.5 * (incorrectSelected / totalIncorrect)`, clamped to `[0,1]`; single-select and free-response remain binary `0` or `1`. Session snapshots persist `result.score`, results aggregate fractional points, and MCQ memory events/profile evidence carry `score`, `max_score`, `partial_credit`, plus session aggregate `score`, `max_score`, and `partial_credit_count`. `POST /api/v1/mcq/evaluate` runs binary semantic evaluation for written responses and returns concise feedback. Raw written answers remain in resumable session state and are never copied into memory events. Generated coding problems (`kind: problem`) run through `problemverify` before `PersistGenerated`: an internal sandbox path executes the reference solution against the server-built `CODEGYM_RESULT` harness (no user `/submissions`), requires all-pass, and allows a bounded LLM adjudicate/repair loop before rejecting non-converging packages (`docs/ai-prompts/04-problem-verification.md`). Marathon uses a stable `generation_failed` phase (aligned with memory event `generation_failed`) with Retry, Edit prompt, and explicit Use practice set; it never silently swaps in the built-in set. Intake prompt behavior mirrors `docs/ai-prompts/01-intake-clarifying-agent.md`, MCQ mirrors `docs/ai-prompts/05-mcq-marathon.md`, and evaluation mirrors `docs/ai-prompts/09-mcq-free-response-evaluation.md`; keep them in sync.
- Agentic problem generation isolation: opencode v1.18.11 starts a fresh logical session for each plain `opencode run`, but a shared HOME exposes prior prompts in `~/.local/share/opencode/opencode.db`, and same-user working directories remain readable through default tools or shell. Per-agent permission denies are defense-in-depth tool dispatch, not an OS filesystem boundary. For the independent spec/tests/reference design, prefer a separate Daytona sandbox per role (only the spec copied into tests/reference); separate Unix users plus private filesystem modes are the minimum same-sandbox fallback. Never rely on separate sessions, `--dir`, or separate same-user HOME paths as the load-bearing isolation guarantee. Evidence: `spikes/opencode/ISOLATION.md`.
- Conversational generation uses `generation.Streamer` beside the structured `Generator`; the OpenAI-compatible adapter implements both. Interview also passes through the New Practice intake gate before the server generates its opening question.
...
- CI: `.github/workflows/ci.yml` runs the memory-event registry drift check, frontend `npm ci`/lint/Vitest/build, and backend `go test ./...` on PRs/pushes to `codegym-v2`. `.github/workflows/openapi-lint.yml` runs `npm run api:lint` (Redocly) when `api/**` changes. `.github/workflows/nightly.yml` fails closed when `CODEGYM_TEST_DATABASE_URL` or `DAYTONA_API_KEY` is absent and otherwise runs the real Neon and Daytona integration suites; those Actions secrets must exist before the scheduled workflow can be considered operational.
...
base URLs, and stable `VITE_APP_ORIGIN` values. The frontend must redirect an
ephemeral deployment to `VITE_APP_ORIGIN` before mounting Auth0, and the Auth0
provider must use that same origin rather than `window.location.origin`. This
keeps PKCE state and the one allowed stable callback on the same origin. If
frontend and backend are on different origins, confirm CORS and Auth0 allowed
origins together.
...
- Memory-aware MCQ generation and written-answer evaluation are wired through
  the provider-neutral orchestrator. Coding-problem and interview generation
  still need their provider routes and frontend flows.
